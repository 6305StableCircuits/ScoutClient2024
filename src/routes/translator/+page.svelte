    ...
    */
    ...
    const translate = (sessions:Array<any>) => {
        ...
        for(var i = 0; i < sessions.length; i++){
            convertedjson["TeamNumber"] = s[i].team;
            convertedjson["matchNumber"] = s[i].round;
            convertedjson["AutoLeave"] = s[i].left == true ? 2 : 0;
            convertedjson["AutoAmp"] = s[i].advancedScoring.autoamp;
            convertedjson["Notes"] = "Sent from scoutclient2024.netlify.app, extra data: Misses: " + s[i].misses + ", Summary: " + s[i].summary.text;
            convertedjson["totalScore"] = s[i].score;
            convertedjson["TeleFoulValue"] = s[i].foul ? 1 : 0;
            convertedjson["TeleTechFoulValue"] = s[i].techFoul?1:0;
            convertedjson["TeleAmp"] = s[i].advancedScoring.teleamp;
            convertedjson["TeleSpeakerNa"] = s[i].advancedScoring.telespeaker.reg;
            convertedjson["TeleSpeakerA"] = s[i].advancedScoring.telespeaker.charged;
            convertedjson["TelePark"] = s[i].park ? 1 : 0;
            convertedjson["TeleOnStageNoSpot"] = s[i].climb ? 3 : 0;
            convertedjson["TeleOnStageSpto"] = s[i].spotlight ? 4 : 0;
            convertedjson["TeleHarmony"] = s[i].harmony ? 2 : 0;
            convertedjson["TeleTrap"] = s[i].trap ? 5 : 0;
            convertedjson["CoopertitionBonus"] = s[i].coopertition ? 1 : 0;
            convertedjson["RankingMelody"] = s[i].melody ? 1 : 0;
            convertedjson["RankingEnsemble"] = s[i].ensemble ? 1 : 0;
            convertedjson["RankingTie"] = s[i].tie ? 1 : 0;
            convertedjson["RankingWin"] = s[i].win ? 2 : 0;
            convertedjson["Scout"] = s[i].scouter;
            convertedjson["TotalAutoPoints"] = s[i].autoScore;
            convertedjson["TotalTeleOpPoints"] = s[i].teleopScore;
            convertedjson["YellowCardValue"] = s[i].yellowCard ? 1:0;
            convertedjson["RedCardValue"] = s[i].redCard ? 1:0;
            convertedjson["TeleDisabledValue"] = s[i].disabled ? 1:0;
            convertedjson["TeleDisqulaifiedValue"] = s[i].dq ? 1:0;
            convertedjson["AutoSpeakerNa"] = s[i].advancedScoring.autospeaker;
            convertedjson["AutoSpeakerA"] = 0;
            convertedjson["TotalRankingPoints"] = (s[i].tie?1:0) + (s[i].win?2:0) + (s[i].melody?1:0) + (s[i].ensemble?1:0);
            fd = jsonToFormData(convertedjson);
            fetch("https://6305.airofan.com/Home/GamesSave",{
                method:"POST",
                body: fd,
                headers:{
